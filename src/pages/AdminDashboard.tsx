...
import { Settings, DollarSign, MapPin, BookOpen, Bell, Download, Upload, Trash2, CreditCard as Edit, Plus, Save, X, LogOut, Home, Check, AlertCircle, Info, PackageOpen } from 'lucide-react';
...
export function AdminDashboard() {
  ...
  const availableGenres = [
    'Drama', 'Romance', 'Acción', 'Comedia', 'Familia',
    'Thriller', 'Misterio', 'Histórico', 'Fantasía', 'Ciencia Ficción'
  ];
  ...
  const getCountryFlag = (country: string) => {
    const flags: { [key: string]: string } = {
      'Cuba': '🇨🇺', 'Turquía': '🇹🇷', 'México': '🇲🇽', 'Brasil': '🇧🇷',
      'Colombia': '🇨🇴', 'Argentina': '🇦🇷', 'España': '🇪🇸', 'Estados Unidos': '🇺🇸',
      'Corea del Sur': '🇰🇷', 'India': '🇮🇳', 'Reino Unido': '🇬🇧', 'Francia': '🇫🇷',
      'Italia': '🇮🇹', 'Alemania': '🇩🇪', 'Japón': '🇯🇵', 'China': '🇨🇳', 'Rusia': '🇷🇺'
    };
  ...
  const resetNovelForm = () => {
    setNovelForm({
      titulo: '', genero: '', capitulos: 0, año: new Date().getFullYear(),
      descripcion: '', pais: '', imagen: '', estado: 'transmision'
    });
  };
      ...
      <div className="max-w-7xl mx-auto p-6">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                            <div><span className="font-medium">Género:</span> {novel.genero}</div>
                            <div><span className="font-medium">Capítulos:</span> {novel.capitulos}</div>
                            <div><span className="font-medium">Año:</span> {novel.año}</div>
                            <div><span className="font-medium">País:</span> {getCountryFlag(novel.pais || 'No especificado')} {novel.pais || 'No especificado'}</div>
                          </div>
                          {novel.descripcion && (
                    ...
                    <div key={zone.id} className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-green-600 font-semibold">Costo: ${zone.cost.toLocaleString()} CUP</p>
                        </div>
        ...
        {activeTab === 'prices' && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <form onSubmit={(e) => { e.preventDefault(); addNotification('Precios actualizados correctamente', 'success'); }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Precio de Películas (CUP)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Precio de Series por Temporada (CUP)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Precio de Novelas por Capítulo (CUP)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Recargo por Transferencia (%)</label>
                <input
                  type="number"
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-gray-500 text-sm mt-1">{notification.timestamp.toLocaleString()}</p>
                      </div>
                      {!notification.read && (
                        <button
        ...
        {showImportModal && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl w-full max-w-2xl p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Configuración JSON</label>
                  <textarea
                    value={importData}
